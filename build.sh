...
if [[ "x${1}x" == "xx" ]]; then
  ...
  if [[ `expr index "$1" 1` > 0 ]]; then
    make COMPILE=gcc BOOT=new APP=1 SPI_SPEED=40 SPI_MODE=QIO SPI_SIZE_MAP=2 OTAUPGRADE=1 ROMNUM=1
  elif [[ `expr index "$1" 2` > 0 ]]; then
    make COMPILE=gcc BOOT=new APP=2 SPI_SPEED=40 SPI_MODE=QIO SPI_SIZE_MAP=2 OTAUPGRADE=1 ROMNUM=2
  else
